...
As Trident Protect relies on Trident to protect applications configured with FSxN volumes, you also need to create a Volume Snapshot Class that is linked to a Trident driver:
...
In order for the VM creation process to be much faster (a few seconds), you first need to rebuild the source images. +
